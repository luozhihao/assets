...
class RoomChart extends Component {
    ...
    showView1 = () => {
        ...
        return fetch(request)
            .then((res) => { return res.json() })
            .then((data) => {
                this.randerChart('roomTotal', '发行部成本率', '成本率', '%', data.overview, 1, '可点击', 'overview')
            })
    }
    ...
    showView2 = (event, type) => {
        ...
        return fetch(request)
            .then((res) => { return res.json() })
            .then((data) => {
                this.randerChart('roomProduct', '发行部成本率', '成本率', '%', data.overview, 2, '不可点击')
                this.randerChart('roomIncome', '发行部收入', '金额', '元', data.income, 2, '不可点击')
                this.randerChart('roomPay', '发行部支出', '金额', '元', data.expenditure, 2, '可点击')
            })
    }
    ...
    showView3 = (event) => {
        ...
        return fetch(request)
            .then((res) => { return res.json() })
            .then((data) => {
                this.randerChart('roomPayDetails', '支出明细', '金额', '元', data.expenditure, 3, '不可点击')
            })
    }
